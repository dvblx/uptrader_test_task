...
def get_child_items(items_values, current_item_id, selected_item_id_list):
    item_list = [item for item in items_values.filter(mi_parent_id=current_item_id)]
...
def draw_menu(context, menu):
    try:
        items = MenuItem.objects.filter(menu__menu_name=menu)
        items_values = items.values()
        primary_item = [item for item in items_values.filter(mi_parent=None)]
        ...
        result_dict = {
            'items': [
                item for item in MenuItem.objects.filter(menu__menu_name=menu, mi_parent=None).values()
                ]
            }
